...
export const getItemGuideData = async (taskCode) => {
    return new Promise((resolve, reject) => {
        api.GetItemGuide({
            ...
            // 将数据处理为有序格式
            resolve(standardizingItemGuideData(response.data.data))
        }).catch(error => {
            reject(error)
        })
    })
...
export const getItemGuideOnDetailFormat = async (taskCode) => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            // 转化为符合事项详情展示格式的数据
            resolve(standardItemGuideToDetailFormat(detailData))
        } catch (err) {
            reject(err)
        }
    })
...
/**
 * 将标准的事项指南数据格式转化为事项详情弹窗展示需要的数据格式。
 * @param detailData {{
 *     taskName: string,
 *     taskCode: string,
 *     serviceAgentName: string,
 ...
 * }]}
 */
export const standardItemGuideToDetailFormat = (detailData) => {// 将 detailData 中的申办材料数组 materials 变成我们想要展示的格式。
    let materials = <></>
