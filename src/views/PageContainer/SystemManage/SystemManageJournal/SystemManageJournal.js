...
const SelectForm = (props) => {
  ...
  const Search = () => {
    if (myself) {
      ...
      setMyself(localStorage.getItem('_id'))
    }
    const data = {
      myself,
      today,
        ...
        <Form layout={"inline"} form={underForm}>
          <Form.Item>
            <Space>
              ...
              <Checkbox onChange={(e)=>{setMyself(e.target.checked)}} checked={myself}>
                查询操作人为您
              </Checkbox>
              ...
              <Form.Item>
                <Button htmlType="button" onClick={()=>{topForm.resetFields();setMyself(false);setToday(false);setThisWeek(false);console.log(myself,today,thisWeek);setGroupVal(0);Search()}}>重置</Button>
              </Form.Item>
