...
const UnitList = () => {
    ...
    const getUnitAndRefreshTree = useCallback(() => {
        api.GetUnit().then((res) => {
            console.log('getUnit', res.data.data)
    ...
    const showDeleteConfirm = useCallback((nodeData) => {
        confirm({
            ...
            {/*由于异步加载数据晚于 tree 的默认展开，会导致默认展开的设置无效。通过增加一个判断来让获取数据之后再展示 tree*/}
            {
                treeData.length > 0
                    ...
                    <Tree
                        ...
                        defaultExpandedKeys={[1653018366962]} // 默认展开的节点
